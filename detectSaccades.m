...
for k = 1:max(velLabeled)
    ...
    ETparams.saccadeInfo(i, j, kk).Xstart = ETparams.data(i,j).X(saccadeStartIdx);
    ETparams.saccadeInfo(i, j, kk).Ystart = ETparams.data(i,j).Y(saccadeStartIdx);
    ETparams.saccadeInfo(i, j, kk).Xend = ETparams.data(i,j).X(saccadeEndIdx);
    ETparams.saccadeInfo(i, j, kk).Yend = ETparams.data(i,j).Y(saccadeEndIdx);
    ETparams.saccadeInfo(i, j, kk).AmplitudePx = sqrt(((ETparams.data(i, j).X(saccadeEndIdx) - ...
                                                (ETparams.data(i, j).X(saccadeStartIdx))))^2 + ...
                                               ((ETparams.data(i, j).Y(saccadeEndIdx) - ...
                                                (ETparams.data(i, j).Y(saccadeStartIdx))))^2);
    ETparams.saccadeInfo(i, j, kk).AmplitudeDeg = ETparams.saccadeInfo(i, j, kk).AmplitudePx/... % AmplitudeDeg converts AmplitudePx to degree
        (nanmedian(ETparams.data(i,j).pxPerDeg(saccadeStartIdx : saccadeEndIdx))); % using the median ETparams.data(i,j).pxPerDeg during the saccade
   ...
   % If glissade detected    
       ...
       % Detect end.
        ...
        if glissadeDuration > 2 * ETparams.minFixDur | any(ETparams.nanIdx(i,j).Idx(glissadeIdx))
            ETparams.glissadeInfo(i, j, kk).type = 0; 
            ETparams.glissadeInfo(i, j, kk).duration = 0;
            glissadeEndStrongIdx = [];
        else
